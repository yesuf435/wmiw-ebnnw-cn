...
define('SITEDIR', _dir_path(substr(dirname(__FILE__), 0, -19)));
define("VERSION", '昂酷网络(ONCOO.NET)');
$sqlFile = 'database.sql';
$configFile = 'config.php';
if (!file_exists(SITEDIR . 'Application/install/' . $sqlFile)) {
    echo '缺少数据库文件!';
    exit;
}
$Title = "昂酷网络（ONCOO.NET）WEB安装程序";
$Powered = "Powered by WWW.ONCOO.NET";
$steps = array(
    '1' => '安装许可协议',
    '2' => '运行环境检测',
    '3' => '安装参数设置',
...
switch ($step) {
    ...
    case '2':
        ...
        if (phpversion() < 5) {
            die('本系统需要PHP5.3+MYSQL >=5.3.0环境，当前PHP版本为：' . phpversion());
        }
        ...
        $folder = array('Application/Cache', 'Application/Common/Conf', 'Application/Database', 'Application/Runtime', 'Uploads');
    ...
    case '4':
        if (intval($_GET['install'])) {
            ...
            $cache_fix = randCode(6);
            $cache_frefix = randCode(6);
            $config = array();
            $config['SITE_INFO']['name'] = addslashes(trim($_POST['sitename'])); //网站名称
            $config['SITE_INFO']['title'] = addslashes(trim($_POST['sitetitle'])); //网站标题
            ...
            //读取数据文件
            $sqldata = file_get_contents(SITEDIR . 'Application/install/' . $sqlFile);
            ...
            $time = time();
            $query = "INSERT INTO `{$dbPrefix}admin` VALUES (1, '超级管理员', '".$email."','','".$password."',1,  '我是超级管理员','', ".$time.");";
            mysql_query($query);
            $message = '成功添加管理员<br />成功写入配置文件<br>安装完成．';

            file_put_contents(SITEDIR . "Application/Common/Conf/systemConfig.php", ("<?php\treturn " . var_export($config, true) . ";?>"));
